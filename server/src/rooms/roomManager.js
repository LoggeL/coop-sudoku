...
class RoomManager {
    ...
    createEmptyBoard(initialBoard) {
        return initialBoard.map((row) => row.map((val) => ({
    ...
    createRoom(roomId, playerName, difficulty, mode, playerId) {
        const { board: initialBoard, solution } = sudoku_1.SudokuEngine.generate(difficulty);
        const board = this.createEmptyBoard(initialBoard);
        const player = {
            id: playerId,
            name: playerName,
    ...
    joinRoom(roomId, playerName, playerId) {
        ...
        if (room.mode === 'versus' && room.playerBoards) {
            const { board: initialBoard } = sudoku_1.SudokuEngine.generate(room.gameState.difficulty);
            // Use the same puzzle (solution) - recreate from the stored solution
            room.playerBoards[playerId] = room.gameState.board.map((row, r) => row.map((cell, c) => ({
    ...
    makeMove(playerId, row, col, value) {
        ...
        if (room.mode === 'versus' && player.finished)
            return null;
        if (room.mode === 'versus') {
    ...
    makeMoveCoop(room, player, playerId, row, col, value) {
    ...
    makeMoveVersus(room, player, playerId, row, col, value) {
        if (!room.playerBoards || !room.claimedCells)
            return null;
        ...
        if (cell.initial)
            return null;
        if (cell.value !== null)
            return null; // Cell already filled in versus
        if (value === null) {
    ...
    toggleNote(playerId, row, col, note) {
        ...
        let board;
        if (room.mode === 'versus') {
            if (!room.playerBoards || !room.playerBoards[playerId])
                return null;
    ...
    clearNotesForNumber(board, row, col, value) {
        ...
        for (let r = blockRowStart; r < blockRowStart + 3; r++) {
            for (let c = blockColStart; c < blockColStart + 3; c++) {
                ...
                if (idx !== -1)
                    board[r][c].notes.splice(idx, 1);
            }
        }
    }
    checkWin(board, solution) {
        for (let r = 0; r < 9; r++) {
            for (let c = 0; c < 9; c++) {
